...
export function addMineWorksBase(d, parent, era) {
  ...
  const drum = d.group(machinery, -3.65, 0.65, 1.4);
  drum.name = 'Working hoist drum';
  drum.userData.animated = true;
  d.mesh(drum, 'cylinder', [0.4, 0.6, 0.4], [0, 0, 0], '#53635c').rotation.z = Math.PI / 2;
  d.rod(drum, [-0.38, 0, 0], [-0.38, 0.55, 0], 0.04, a.frame);
  d.rod(drum, [-0.38, 0.55, 0], [-0.7, 0.55, 0], 0.045, a.frame);
  root.userData.hoistUpdate = (time) => {
    drum.rotation.x = time * 0.6;
  };
  if (a.machine !== 'hand') {
